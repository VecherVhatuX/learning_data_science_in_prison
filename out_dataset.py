...
def create_model(embedding_size=128, fully_connected_size=64, dropout_rate=0.2, max_sequence_length=512, learning_rate_value=1e-5):
...
def train_model(model, train_dataset, test_dataset, epochs=5, learning_rate_value=1e-5):
...
def run_pipeline(dataset_path, snippet_folder_path, num_negatives_per_positive=1, batch_size=32, epochs=5):
    ...
    train_data = Dataset(train_triplets, 512, Tokenizer(), batch_size=batch_size, epochs=epochs)
    test_data = Dataset(test_triplets, 512, Tokenizer(), batch_size=batch_size, epochs=epochs)
    model = create_model()
    train_model(model, train_data, test_data, epochs=epochs)
