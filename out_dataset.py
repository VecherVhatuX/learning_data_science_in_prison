...
class ModelCreator:
    def create_model(self, vocab_size, embedding_dim, max_sequence_length):
...
class Trainer:
    ...
    def train(self, model, train_dataset, test_dataset, epochs):
        train_losses = []
        test_losses = []
        train_accuracies = []
        test_accuracies = []
...
def main():
    ...
    instance_id_map, snippets = data_loader.load_data(dataset_path, snippet_folder_path)
    triplets = data_loader.create_triplets(instance_id_map, snippets)
    ...
    model = model_creator.create_model(train_dataset.vocab_size, 128, 512)
    
    trainer = Trainer()
    train_losses, test_losses, train_accuracies, test_accuracies = trainer.train(model, train_data, test_data, 5)
    
    plotter = Plotter()
    plotter.plot(train_losses, test_losses, train_accuracies, test_accuracies)
