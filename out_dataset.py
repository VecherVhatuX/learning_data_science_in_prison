...
# Extract all text entries from the triplet dataset
def gather_texts(triplet_data):
    return [text for item in triplet_data for text in (item['anchor'], item['positive'], item['negative'])]

# Convert text sequences into tensor representations using a tokenizer
...
# Randomly rearrange the order of data samples
def shuffle_data(data_samples):
    random.shuffle(data_samples)
    return data_samples

# Create triplet combinations from provided data samples
...
# Load JSON data and prepare necessary mappings
...
# Prepare the dataset by generating triplets from the loaded data
...
# Neural network model for triplet loss training
...
# Compute the triplet loss for training
...
# Count the number of correct matches in the model's predictions
...
def visualize_embeddings(model, data_loader):
    ...
    for batch in data_loader:
        anchor_embeds, _, _ = model(batch['anchor_seq'], batch['positive_seq'], batch['negative_seq'])
        embeddings.extend(anchor_embeds.numpy())
