...
def retry_command(agent, command, attempt, max_attempts):
    ...
    time.sleep(5)
    return retry_command(agent, command, attempt + 1, max_attempts)

def run_with_agent(command, max_attempts):
    tools = setup_tools()
    agent = create_agent(tools=tools)
    return retry_command(agent, command, 0, max_attempts)
...
def begin_process(command, max_attempts):
    ...
    return run_with_agent(command, max_attempts)
...
def execute(command, max_attempts=5, countdown_time=0):
    ...
    return begin_process(command, max_attempts)
