...
class Trainer:
    ...
    def _prepare_data(self, chat_template, data):
        return {
    ...
    def _load_data(self):
        ...
        chat_template = self.model_args.chat_template if self.model_args.chat_template != "none" else ""
        return self._prepare_data(chat_template, train_data), self._prepare_data(chat_template, test_data)

    def _create_train_state(self, rng, model, learning_rate):
    ...
    def _calculate_loss(self, params, batch, use_triplet):
        model = NeuralNetwork()
        if use_triplet:
    ...
    def _execute_train_step(self, state, batch, use_triplet):
        grads = jax.grad(self._calculate_loss, argnums=0)(state.params, batch, use_triplet)
        state = state.apply_gradients(grads=grads)
        return state

    def _train_epoch(self, model, state, dataset, use_triplet):
        return jax.jit(lambda state, dataset: jax.lax.fori_loop(
            0, len(dataset), lambda i, state: self._execute_train_step(state, next(dataset), use_triplet), state
        ))(state, dataset)
    ...
    def train(self):
        ...
        state = self._create_train_state(rng, model, 0.001)

        for epoch in range(self.training_args.num_train_epochs):
            state = self._train_epoch(model, state, train_dataset, self.model_args.use_triplet_loss_trainer)
            print(f"Epoch {epoch+1}")

if __name__ == "__main__":
    model_args = Configuration.Model(model_identifier="t5-base", chat_template="none", use_triplet_loss_trainer=True)
    data_args = Configuration.TrainingData(dataset_name="timdettmers/openassistant-guanaco")
    training_args = Configuration.Training(output_dir="./results", num_train_epochs=3, per_device_train_batch_size=16)
