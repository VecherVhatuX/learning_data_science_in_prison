...
create_triplet_network = lambda embedding_size, vocab_count: Model(
    ...
    layers.Dense(vocab_count)(layers.Dense(embedding_size)(layers.LSTM(embedding_size, return_sequences=True)(layers.Embedding(vocab_count, embedding_size)(inputs))[:, -1, :])
)

calculate_triplet_loss = lambda anchor, positive, negative: tf.reduce_mean(
    tf.maximum(tf.reduce_mean(tf.square(anchor - positive), axis=-1) - tf.reduce_mean(tf.square(anchor - negative), axis=-1) + 2.0, 0.0)
)
...
DataHandler = lambda data, config, tokenizer: {
    ...
    "fetch_sample": lambda self, idx: (
        ...
        [self["tokenizer"].encode(self["data"][random.choice([j for j in range(len(self["data"])) if j != self["indices"][idx]])]['input'], max_length=512, padding='max_length', truncation=True) for _ in range(self["config"]["negative_samples_per_batch"])]
    ),
    "generate_batch_samples": lambda self: [self["fetch_sample"](i) for i in range(len(self["data"]))]
}

BatchGenerator = lambda dataset, config, tokenizer: {
...
add_lr_scheduler = lambda optimizer, config: tf.keras.optimizers.Adam(learning_rate=tf.keras.optimizers.schedules.ExponentialDecay(initial_learning_rate=0.001, decay_steps=1000, decay_rate=0.9))
