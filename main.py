...
class Dataset:
    def __init__(self, data_args, use_triplet):
        self.train_data = load_json_file("train.json")
        self.test_data = load_json_file("test.json")
        self.chat_template = data_args.chat_template if data_args.chat_template != "none" else ""
        self.prepare_data_fn = prepare_data(data_args, self.chat_template)
...
def run_pipeline(model_args, data_args, training_args):
    ...
    for epoch in range(training_args.num_train_epochs):
        for batch in data_loader:
            state = train_step(state, batch, model_args.use_triplet_loss_trainer)
