    ...
    for _ in range(epochs):
        ...
        for anchor, pos, neg in data_loader:
            with tf.GradientTape() as tape:
                loss = calculate_triplet_loss(model(anchor), model(pos), model(neg)) + 0.01 * sum(tf.norm(p, ord=2) for p in model.trainable_variables)
...
if __name__ == "__main__":
    ...
    dataset = DataSampler(data, labels, 5)
