...
def get_triplet_batch(idx, samples, labels, batch_size, num_negatives):
...
def create_triplet_dataset(samples, labels, batch_size, num_negatives):
    ...
    return tf.data.Dataset.from_generator(
        lambda: (get_triplet_batch(idx, samples, labels, batch_size, num_negatives) for idx in range(-(-len(samples) // batch_size))),
    ...
    def train(self, dataset, epochs):
        for epoch in range(epochs):
            ...
            for inputs in dataset:
                loss = train_step(self.model, self.optimizer, inputs, self.margin)
