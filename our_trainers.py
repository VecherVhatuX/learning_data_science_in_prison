...
def create_network(embedding_dim, num_features):
...
def create_input_dataset(input_ids):
...
def train(model, optimizer, dataset, epochs, batch_size):
    for epoch in range(epochs):
        total_loss = 0.0
        dataset.on_epoch_end()
        data_loader = DataLoader(dataset, batch_size=batch_size, shuffle=False)
        for i, batch in enumerate(data_loader):
            total_loss += train_step(model, optimizer, batch).item()
