import tensorflow as tf
import numpy as np
import matplotlib.pyplot as plt
from sklearn.manifold import TSNE
import random
from tensorflow.keras import layers, models, optimizers, losses, datasets

class Embedder(tf.keras.Model):
    def __init__(self, emb_dim, feat_dim):
        super(Embedder, self).__init__()
        # Create an embedding layer to transform input indices into dense vectors
        self.emb_layer = layers.Embedding(emb_dim, feat_dim)
        # Use global average pooling to reduce the dimensionality of the embeddings
        self.avg_pool = layers.GlobalAveragePooling1D()
        # Apply a fully connected layer to transform pooled output
        self.fc_layer = layers.Dense(feat_dim)
        # Apply batch normalization for stable training
        self.batch_norm = layers.BatchNormalization()
        # Normalize the output of the fully connected layer
        self.layer_norm = layers.LayerNormalization()

    def call(self, input_data):
        # Forward pass through the embedding layer
        embedded = self.emb_layer(input_data)
        # Reduce dimensionality with average pooling
        pooled = self.avg_pool(embedded)
        # Normalize the output after applying fully connected layer and batch normalization
        normalized = self.layer_norm(self.batch_norm(self.fc_layer(pooled)))
        return normalized


class TripletData(tf.data.Dataset):
    def __new__(cls, samples, labels, neg_count):
        # Create a dataset from samples and labels
        dataset = tf.data.Dataset.from_tensor_slices((samples, labels))
        dataset = dataset.map(lambda anchor_sample, anchor_label: (
            anchor_sample, 
            tf.convert_to_tensor(random.choice(samples[np.where(labels == anchor_label)[0]])),
            tf.convert_to_tensor(random.sample(samples[np.where(labels != anchor_label)[0]].tolist(), neg_count))
        ))
        return dataset


def triplet_loss(margin=1.0):
    # Define the triplet loss function
    def loss_fn(anchor, positive, negative):
        # Compute distance between anchor and positive samples
        pos_distance = tf.norm(anchor - positive, axis=1)
        # Compute distance between anchor and negative samples
        neg_distance = tf.norm(anchor[:, tf.newaxis] - negative, axis=2)
        # Calculate and return the average triplet loss
        return tf.reduce_mean(tf.maximum(pos_distance - tf.reduce_min(neg_distance, axis=1) + margin, 0.0))
    return loss_fn


def train(model, dataset, epochs, lr):
    # Set up the optimizer for training
    optimizer = optimizers.Adam(learning_rate=lr)
    loss_fn = triplet_loss()  # Initialize the triplet loss function
    loss_history = []

    for epoch in range(epochs):
        epoch_loss = 0
        for anchors, positives, negatives in dataset.batch(32):
            with tf.GradientTape() as tape:
                loss = loss_fn(anchors, positives, negatives)  # Compute the loss for the current batch
            grads = tape.gradient(loss, model.trainable_variables)  # Get gradients
            optimizer.apply_gradients(zip(grads, model.trainable_variables))  # Update model parameters
            epoch_loss += loss.numpy()
        loss_history.append(epoch_loss / len(dataset))  # Store average loss for the epoch
    return loss_history


def evaluate_model(model, samples, labels, k=5):
    # Evaluate the model's performance on the provided samples and labels
    model.evaluate(samples, labels)
    embeddings = model(tf.convert_to_tensor(samples, dtype=tf.int32))  # Get the embeddings
    metrics = calculate_knn_metrics(embeddings.numpy(), labels, k)  # Calculate KNN metrics
    display_metrics(metrics)  # Display calculated metrics
    plot_embeddings(embeddings.numpy(), labels)  # Visualize embeddings


def display_metrics(metrics):
    # Output the performance metrics
    print(f"KNN Accuracy: {metrics[0]:.4f}")
    print(f"KNN Precision: {metrics[1]:.4f}")
    print(f"KNN Recall: {metrics[2]:.4f}")
    print(f"KNN F1-score: {metrics[3]:.4f}")


def generate_data(size):
    # Generate random sample data and corresponding labels
    return np.random.randint(0, 100, (size, 10)), np.random.randint(0, 2, size)


def save_model(model, path):
    # Save the model's weights to the specified path
    model.save_weights(path)


def load_model(model_class, path):
    # Load weights into a newly instantiated model
    model = model_class()
    model.load_weights(path)
    return model


def get_embeddings(model, input_data):
    # Retrieve embeddings for the provided input data
    return model(tf.convert_to_tensor(input_data, dtype=tf.int32)).numpy()


def plot_embeddings(embeddings, labels):
    # Visualize embeddings in 2D space using t-SNE
    tsne = TSNE(n_components=2)
    reduced_data = tsne.fit_transform(embeddings)
    plt.figure(figsize=(8, 8))
    scatter = plt.scatter(reduced_data[:, 0], reduced_data[:, 1], c=labels, cmap='viridis')
    plt.colorbar(scatter)
    plt.show()


def calculate_knn_metrics(embeddings, labels, k=5):
    # Calculate KNN performance metrics based on embeddings
    distance_matrix = np.linalg.norm(embeddings[:, np.newaxis] - embeddings, axis=2)
    nearest_indices = np.argsort(distance_matrix, axis=1)[:, 1:k + 1]
    
    tp_count = np.sum(labels[nearest_indices] == labels[:, np.newaxis], axis=1)
    precision = np.mean(tp_count / k)  # Calculate precision
    recall = np.mean(tp_count / np.sum(labels == labels[:, np.newaxis], axis=1))  # Calculate recall
    
    accuracy = np.mean(np.any(labels[nearest_indices] == labels[:, np.newaxis], axis=1))  # Calculate accuracy
    f1_score = 2 * (precision * recall) / (precision + recall) if (precision + recall) > 0 else 0  # Calculate F1 score
    
    return accuracy, precision, recall, f1_score


def plot_training_loss(loss_history):
    # Plot the training loss over epochs
    plt.figure(figsize=(10, 5))
    plt.plot(loss_history, label='Loss', color='blue')
    plt.title('Loss Over Epochs')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.show()


def execute_training_pipeline(lr, batch_size, epochs, neg_count, emb_dim, feat_dim, data_size):
    # Main function to execute the training pipeline
    samples, labels = generate_data(data_size)  # Generate synthetic data
    triplet_data = TripletData(samples, labels, neg_count)  # Create triplet dataset
    model = Embedder(emb_dim, feat_dim)  # Initialize the embedding model
    loss_history = train(model, triplet_data, epochs, lr)  # Train the model
    save_model(model, "triplet_model.h5")  # Save the trained model
    plot_training_loss(loss_history)  # Visualize the training loss
    evaluate_model(model, samples, labels)  # Evaluate the trained model


def create_additional_data(size):
    # Generate additional synthetic data
    return np.random.randn(size, 10), np.random.randint(0, 2, size)


def plot_embedding_distribution(embeddings):
    # Visualize the distribution of embedding values as a histogram
    plt.figure(figsize=(10, 6))
    plt.hist(embeddings.flatten(), bins=30, alpha=0.7, label='Embedding Values Distribution')
    plt.title('Embedding Values Distribution Histogram')
    plt.xlabel('Value')
    plt.ylabel('Frequency')
    plt.legend()
    plt.show()


def visualize_model_architecture(model):
    # Display the architecture of the model
    model.summary()


if __name__ == "__main__":
    execute_training_pipeline(1e-4, 32, 10, 5, 101, 10, 100)  # Run the training process
    visualize_model_architecture(Embedder(101, 10))  # Show the model summary