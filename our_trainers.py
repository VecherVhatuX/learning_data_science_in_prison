...
class TripletDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        negative_input_ids = self.samples[torch.tensor(negative_indices, dtype=torch.long)].numpy().reshape(self.batch_size, self.num_negatives, -1)
...
class TripletModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        negative_embeddings = self.embed(torch.tensor(negative_input_ids, dtype=torch.long).view(-1, input_ids.shape[1]))

        return anchor_embeddings, positive_embeddings, negative_embeddings

def triplet_loss(anchor, positive, negative):
    return torch.mean(torch.clamp(torch.norm(anchor - positive, dim=1) - torch.norm(anchor.unsqueeze(1) - negative, dim=2) + 1.0, min=0.0))
